...
def copy_repo_file_to_tmp(repo_name, repo_file):
    ...
    logger.debug(f"repo file is {repo_file_path}")
    ...
    logger.debug(f"tmp repo file is {tmp_repo_file_path}")
...
def modify_tmp_repo_file_with_extra_param(tmp_repo_file_path, extra_param):
    ...
    logger.debug(f"file modified content is {file_content}")
    # 重新写入文件
